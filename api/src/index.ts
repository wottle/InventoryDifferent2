...
const ts = () => new Date().toLocaleString('en-US', { dateStyle: 'short', timeStyle: 'medium', hour12: false });
...
`);
    ...
    async function processDevice(
        ...
        const deviceCreateData = {
            ...
            cpuType: deviceData.cpuType ?? deviceData.cpu,
            cpuSpeed: deviceData.cpuSpeed,
            ram: deviceData.ram,
            graphicsChip: deviceData.graphicsChip ?? deviceData.graphics,
            screenSize: deviceData.screenSize,
            displayType: deviceData.displayType,
            displayVariant: deviceData.displayVariant,
            nativeResolution: deviceData.nativeResolution,
            isWifiEnabled: deviceData.isWifiEnabled,
            pramBatteryInstalled: deviceData.pramBatteryInstalled ?? !(deviceData.isPramBatteryRemoved ?? false),
            pramBatteryExpiryDate: deviceData.pramBatteryExpiryDate ? new Date(deviceData.pramBatteryExpiryDate) : null,
    ...
    app.use((err: any, req: express.Request, res: express.Response, next: express.NextFunction) => {
        if (err instanceof multer.MulterError) {
            if (err.code === 'LIMIT_FILE_SIZE') {
                return res.status(400).json({ error: 'File too large. Maximum size is 2 GB.' });
            }
