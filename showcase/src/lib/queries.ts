...
export const GET_ALL_SHOWCASE_JOURNEYS_ADMIN = gql`
  query GetAllShowcaseJourneysAdmin {
    showcaseAllJourneys {
      id title slug description coverImagePath sortOrder effectiveVolumeNumber published publishedAt
      chapters {
        id title
        devices { id }
      }
...
export const UPDATE_JOURNEY = gql`
  mutation UpdateJourney($id: ID!, $input: JourneyInput!) {
    updateJourney(id: $id, input: $input) {
      id title slug description coverImagePath sortOrder effectiveVolumeNumber published
    }
  }
`;
