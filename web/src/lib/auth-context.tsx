...
export function AuthProvider({ children }: { children: ReactNode }) {
    ...
    // Login function
    const login = useCallback(async (username: string | null, password: string): Promise<{ success: boolean; error?: string }> => {
        try {
            const body: { username?: string; password: string } = { password };
            if (username) {
